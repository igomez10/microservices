...
const CreateUserToRoleWithID = `-- name: CreateUserToRoleWithID :one
INSERT INTO users_to_roles (
	id, user_id, role_id
) VALUES (
	$1, $2, $3
...
type CreateUserToRoleWithIDParams struct {
	ID     int64 `json:"id"`
	UserID int64 `json:"user_id"`
	RoleID int64 `json:"role_id"`
}

func (q *Queries) CreateUserToRoleWithID(ctx context.Context, db DBTX, arg CreateUserToRoleWithIDParams) (UsersToRole, error) {
	row := db.QueryRow(ctx, CreateUserToRoleWithID, arg.ID, arg.UserID, arg.RoleID)
	var i UsersToRole
	err := row.Scan(&i.ID, &i.RoleID, &i.UserID)
	return i, err
}
...
func (q *Queries) CreateUserWithID(ctx context.Context, db DBTX, arg CreateUserWithIDParams) (User, error) {
	row := db.QueryRow(ctx, CreateUserWithID,
		arg.ID,
		arg.Username,
		arg.HashedPassword,
		arg.Salt,
...
const GetLatestEventVersionForAggregate = `-- name: GetLatestEventVersionForAggregate :one
SELECT COALESCE(MAX(version), 0)::BIGINT AS version
FROM events
WHERE aggregate_id = $1 AND aggregate_type = $2
`

type GetLatestEventVersionForAggregateParams struct {
	AggregateID   int64  `json:"aggregate_id"`
	AggregateType string `json:"aggregate_type"`
}

func (q *Queries) GetLatestEventVersionForAggregate(ctx context.Context, db DBTX, arg GetLatestEventVersionForAggregateParams) (int64, error) {
	row := db.QueryRow(ctx, GetLatestEventVersionForAggregate, arg.AggregateID, arg.AggregateType)
	var version int64
