...
func (s *UserApiService) CreateUser(ctx context.Context, user openapi.User) (openapi.ImplResponse, error) {
	...
	pwdUUID, err := uuid.NewRandom()
	if err != nil {
		log.Error().
			Err(err).
			Str("X-Request-ID", ctx.Value("X-Request-ID").(string)).
			Msg("Error generating random string for expired password")
		return openapi.Response(http.StatusInternalServerError, nil), nil
	}
	saltedPassword := pwdUUID.String() + salt
	hashedPassword := sha256.Sum256([]byte(saltedPassword))
	expiredPassword := base64.StdEncoding.EncodeToString([]byte(hashedPassword[:]))

	// create the user with an autogenerated password that already expired
	passwordExpiresAt := time.Now().Add(-time.Hour * 100)
	params := db.CreateUserParams{
		Username:                user.Username,
		FirstName:               user.FirstName,
		LastName:                user.LastName,
		Email:                   user.Email,
		HashedPassword:          expiredPassword,
		HashedPasswordExpiresAt: passwordExpiresAt,
		Salt:                    salt,
	}
