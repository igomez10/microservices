...
func TestCreateUser(t *testing.T) {
	...
	t.Run("verify a user doesnt exist yet", func(t *testing.T) {
		...
		if err == nil {
			...
			fmt.Fprintf(os.Stderr, "Expected error when calling `UserApi.GetUserByUsername`: %v\n", err)
	...
	t.Run("validate user was created", func(t *testing.T) {
...
func TestFollowCycle(t *testing.T) {
	...
	t.Run("user1 follows user2", func(t *testing.T) {
		...
		followers, r2, err2 := apiClient.UserApi.GetUserFollowers(context.Background(), username2).Execute()
		if err != nil {
			t.Errorf("Error when calling `UserApi.FollowUser`: %v\n %+v\n", err2, r2)
			fmt.Fprintf(os.Stderr, "Error when calling `UserApi.FollowUser``: %v\n", err2)
			fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	...
	t.Run("user1 unfollows user2", func(t *testing.T) {
