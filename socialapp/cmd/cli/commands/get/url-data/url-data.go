...
func GetCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			httpClient, err := auth.GetHTTPClient(ctx, envName, username, password, []string{"socialapp.urls.read"})
			if err != nil {
			...
			urlData, httpResponse, err := apiClient.URLAPI.GetUrlData(ctx, alias).Execute()
			if err != nil {
			...
			urlJSON, err := json.MarshalIndent(urlData, "", "  ")
			if err != nil {
				return err
			}
