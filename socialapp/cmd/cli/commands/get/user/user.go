...
func GetCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			httpClient, err := auth.GetHTTPClient(ctx, envName, username, password, []string{"socialapp.users.read"})
			if err != nil {
			...
			user, res, err := clnt.UserAPI.GetUserByUsername(ctx, targetUsername).Execute()
			if err != nil {
				fmt.Printf("Full HTTP response: %v\n", res)
				return fmt.Errorf("error when calling `UserAPI.GetUserByUsername`: %v", err)
			}
