...
func GetCmd() *cli.Command {
	return &cli.Command{
		Name:      "user-comments",
		Usage:     "get all comments for a user",
		ArgsUsage: "<username>",
		Flags: []cli.Flag{
			&cli.StringFlag{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			comments, res, err := clnt.UserAPI.GetUserComments(ctx, targetUsername).
				Limit(int32(pagesize)).
				Offset(int32(offset)).
				Execute()
			if err != nil {
				fmt.Printf("Full HTTP response: %v\n", res)
				return fmt.Errorf("error when calling `UserAPI.GetUserComments`: %v", err)
			}
