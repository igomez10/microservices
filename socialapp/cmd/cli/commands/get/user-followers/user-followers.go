...
func GetCmd() *cli.Command {
	return &cli.Command{
		Name:      "user-followers",
		Usage:     "get all followers for a user",
		ArgsUsage: "<username>",
		Flags: []cli.Flag{
			&cli.StringFlag{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			followers, res, err := clnt.UserAPI.GetUserFollowers(ctx, targetUsername).Execute()
			if err != nil {
				fmt.Printf("Full HTTP response: %v\n", res)
				return fmt.Errorf("error when calling `UserAPI.GetUserFollowers`: %v", err)
			}
