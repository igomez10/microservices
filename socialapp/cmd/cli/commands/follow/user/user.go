...
func GetCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			...
			httpClient, err := auth.GetHTTPClient(ctx, envName, username, password, []string{"socialapp.follower.create"})
			if err != nil {
			...
			res, err := clnt.UserAPI.FollowUser(ctx, followedUsername, followerUsername).Execute()
			if err != nil {
				fmt.Printf("Full HTTP response: %v\n", res)
				return fmt.Errorf("error when calling `UserAPI.FollowUser`: %v", err)
			}
